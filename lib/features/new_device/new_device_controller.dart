...
class NewDeviceController extends GetxController {
  ...
  Future<void> connectToDevice(Device device) async {
    try {
      ...
      // Set connecting state immediately so UI shows loading
      // Keep connectingDeviceMac set even during scanning so UI shows connecting state
      bleController.connectingDeviceMac.value = device.macAddress.toUpperCase();
      
      SnackbarHelper.showInfo('Scanning for ${device.displayName}...', title: 'Scanning');
      await bleController.startScan();
      // After scanning, if we're still trying to connect to this device, set state to connecting
      if (bleController.connectingDeviceMac.value?.toUpperCase() == device.macAddress.toUpperCase()) {
        bleController.connectionState.value = BleConnectionState.connecting;
      }
      ...
      if (matchingDevice == null || matchingDevice.bluetoothDevice == null) {
        ...
        bleController.connectingDeviceMac.value = null;
        bleController.connectionState.value = BleConnectionState.disconnected;
